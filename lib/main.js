$( document ).ready(function() {
    for (var i=0; i != 50; i++) {
        $( "<img src=\"resources/textures/Star.png\">" ).appendTo( ".stars" )
    }
    ...
    function handleMouseMove(event) {
        ...
        for (var i = 0; i != $('.stars > img').length; i++) {
            set_pos ($('.stars > img').get(i), event.pageX, event.pageY);
        }
        set_pos($('#mars').get(0), event.pageX, event.pageY);
        set_pos($('#moon').get(0), event.pageX, event.pageY);
        set_rot($('.menu').get(0), event.pageX, event.pageY);
    }
});
...
function set_rot (e, x, y) { // Use negative for inverse
  $(e).css("transform", "rotateX(" + x * $(e).data ('xfactor') + "deg)" + "rotateY(" + y * $(e).data ('yfactor') + "deg) translateY(-120px)");
}
